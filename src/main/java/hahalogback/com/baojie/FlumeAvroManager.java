...
public class FlumeAvroManager {
	...
	private FlumeAvroManager(final Properties props, final Long reportingWindowReq, final Integer batchSizeReq,
			final Integer reporterMaxThreadPoolSizeReq, final Integer reporterMaxQueueSizeReq,
		...
		final int reporterMaxQueueSize = reporterMaxQueueSizeReq == null ? DEFAULT_REPORTER_MAX_QUEUE_SIZE
				: reporterMaxQueueSizeReq;
	...
	/**
	 * Pulls events from the queue and offloads the resulting array of events to
	 * a thread pool(?) that sends that batch to a flume agent
	 */
			...
			while (!shutdown) {
				...
				try {
					while (count < batchSize && System.currentTimeMillis() < maxTime) {
						lastPoll = Math.max(System.currentTimeMillis(), lastPoll); // Corrects
																					// to
																					// last
																					// seen
																					// time
				...
				if (count > 0) {
					...
					try {
						...
						loggingContext
								.addError(
										"Logging events batch rejected by EventReporter. Check reporter connectivity or "
												+ "consider increasing reporterMaxThreadPoolSize or reporterMaxQueueSize",
										ex);
					}
				}
			}
